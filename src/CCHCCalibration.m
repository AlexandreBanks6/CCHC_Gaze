...
Here we assume first gazelog is the 8-point calibration and the remaining 
gazelogs are the 1-point calibration with the head lift in between.
%}
...
% Define header for the merged "CCHCData_MM-DD-YYYY_HR-MM-SC.csv" 
% This merged data only contains calibration points and essential columns
...
%% Computing CCHC Calibration Parameters
CALIB_THRESHOLD=5; %Number of pupil detections per calibration point needed
CALIB_ONE_DOT_THRESHOLD=3; %Number of single-dot calibrations needed to continue
...
for m=[1:num_dir] %Looping for all directories
    ...
    if isempty(gazelog_files)
        disp(['!!!!No gazelog files in: ',dirnames{m},' cannot run calibration!!!!']);
        continue;
    end

    timestamps=[]; %Will hold timestamps of gazelog files in format that works with datenum
    for j=[1:length(gazelog_files)] %Loops for all gazelog files and extracts timestamp portion
        filename=gazelog_files(j).name;
        % Extract timestamp from filename: gazelog_MM-DD-YYYY_HR-MM-SC.txt
        timestamp_str=extractBetween(filename,'gazelog_','.txt'); 
        timestamp_str=timestamp_str{1}; %Converts from cell to string
        datetime_str=strrep(timestamp_str,'_',' '); %Replaces underscore between date and time with space as expected by datenum
        datetime_str=strrep(datetime_str,'-','/'); %Replace dashes with slashes
        %Split date and time parts
        parts=split(datetime_str,' ');
        time_part=strrep(parts{2},'/',':'); %Replace time slashes with colons
        datetime_str=[parts{1},' ',time_part]; 
        timestamps(j)=datenum(datetime_str,'mm/dd/yyyy HH:MM:SS'); %Stores timestamp as a datenum
    ...
    is_calib_init=true;
    for j=[1:length(gazelog_files)] %Loop for each gazelog file and merge with the eyeCornerData file
        ...
        %Extract gazelog data only when calibration button is pressed
        calib_ind=find(gazelog_data(:,48)==1);
        gazelog_data=gazelog_data(calib_ind,:);
        [r,c]=size(gazelog_data);
        if r<=1 %Need at least one calibration point
            disp(['!!!No calibration points in: ', gazelog_filename]);
            merged_files{j} = [];
            if j==1 %We don't have the initial 8-point calibration data
                is_calib_init=false;
            end
            continue;
        end
        ...
        if isempty(common_frames)
            disp(['!!!No matching frames found between: ', gazelog_filename, ' and ', eyecorner_filename]);
            merged_files{j} = [];
            if j==1 %We don't have the initial 8-point calibration data
                is_calib_init=false;
            end
        ...
        %Store merged filepath for later use
        merged_files{j}=merged_filepath;


    end
    
    %Remove empty entries from the merged_files cell array
    merged_files=merged_files(~cellfun(@isempty, merged_files));

    %Check if we have the inital calibration
    if ~is_calib_init
        disp(['!!!!Do not have adequate 8-point calibration data for: ', dirnames{m}, '; cannot run calibration!!!!']);
        continue;
    end

    %Check if we have enough one-dot calibrations, if it is we continue to next directory
    if length(merged_files)<(1+CALIB_ONE_DOT_THRESHOLD)
        disp(['!!!!Not enough calibration data for: ', dirnames{m}, '; cannot run calibration!!!!']);
        continue;
    end    

    
    %----------------------<Calibration>-----------------------
    calib_init_filename=merged_files{1};
    calib_init_data=readmatrix(calib_init_filename); %Data for naive polynomial regression fit
    check_calib=checkDetection(calib_init_data,CALIB_THRESHOLD); %Checks that we have enough pupil detections
    if (check_calib==true)
        %#######Runs Naive Polynomial Regressor Fitting#######
        ...
        if length(dist_cell)==0 %Not enough data of inter-glint distance
            disp(['!!!!Do not have enough data in initial 8-point calibration data for: ', dirnames{m}, '; cannot run calibration!!!!']);
            continue;
        end
        model_poly=robustRegressor(train_cell); %Gets naive polynomial regressor

        %#######Running CCHC Calibration#########
        calib_onedot_filenames=merged_files(2:end); %One-dot calibration data
        data_cell=cell(length(calib_onedot_filenames), 1); % Initialize cell array
        % Read each file into the cell array
        for k = 1:length(calib_onedot_filenames)
            data_cell{k} = readmatrix(calib_onedot_filenames{k});
        end
        ...
        disp(['!!!!Do not have enough pupil detections in initial 8-point calibration data for: ', dirnames{m}, '; cannot run calibration!!!!']);
        continue;
...
function [reformatted_data_right,reformatted_data_left]=reformatDataEval(eval_data)
    ...
    reformatted_data_right=[eval_data(:,2),glintspupils_right(:,3)-glintspupils_right(:,1),...
        ...
        glintspupils_right(:,8)-glintspupils_right(:,2),eval_data(:,27),eval_data(:,28),eval_data(:,30:33)];
    ...
    reformatted_data_left=[eval_data(:,2),glintspupils_left(:,3)-glintspupils_left(:,1),...
        ...
        glintspupils_left(:,8)-glintspupils_left(:,2),eval_data(:,27),eval_data(:,28),eval_data(:,34:37)];
